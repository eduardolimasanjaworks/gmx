#!/usr/bin/env node
/**
 * Campos GR (gerenciamento de risco) em embarques + auditoria embarque_gr_log.
 *
...
async function main() {
  ...
  await ensureField(token, 'embarques', 'gr_feito', 'boolean', {
    interface: 'boolean',
    note: 'Gerenciamento de risco confirmado pelo atendente',
    width: 'half',
  });
  await ensureField(token, 'embarques', 'gr_feito_em', 'timestamp', {
    interface: 'datetime',
    note: 'Data/hora em que o atendente marcou Feito GR',
    width: 'half',
  });
  await ensureField(token, 'embarques', 'gr_feito_por_nome', 'string', {
    note: 'Nome do atendente que marcou Feito GR',
    width: 'half',
  });
  await ensureField(token, 'embarques', 'gr_feito_por_id', 'string', {
  ...
  await ensureField(token, 'embarque_gr_log', 'embarque_id', 'integer', { required: true });
  await ensureField(token, 'embarque_gr_log', 'acao', 'string', {
    required: true,
  ...
  await ensureField(token, 'embarque_gr_log', 'atendente_nome', 'string', { required: true });
  await ensureField(token, 'embarque_gr_log', 'atendente_id', 'string');
  await ensureField(token, 'embarque_gr_log', 'observacao', 'text', {
    note: 'Motivo obrigatório ao desmarcar',
  });
  await ensurePermissions(token, 'embarque_gr_log');

  console.log('OK — campos GR + auditoria embarque_gr_log');
}
